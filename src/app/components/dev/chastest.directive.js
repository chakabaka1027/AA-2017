...
//
	...
	//each node orgnized by code value
	/** @ngInject */
	//node class

	function dialogNode($log) {
//?= can have an alternative structure if its fixed - verrit the score value in properties
		function DialogNode(data ) {//code, isFinal,
			this.parent = null;
			this.children = {};
			this.isRoot = true;
			...
			// this.choiceCode = code[code.length-1];//gets the last element -
			// this.isFinal = isFinal; //i.e. leaf node
	...
	var testingFile = {
			"node1": [ //first heigth element n + 1
					{
							"code": "A", //FIRST Node
